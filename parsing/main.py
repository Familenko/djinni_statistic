...
def parse(product_element: WebElement) -> Vacancies:
    ...
    requirements_elements = product_element.find_elements(
        By.CSS_SELECTOR, ".job-list-item__job-info .nobr"
    )
    requirements = [element.text.strip() for element in requirements_elements]

    with ChromeWebDriver() as driver2:
        detailed_view = product_element.find_element(
            By.CSS_SELECTOR, ".job-list-item__title a"
        ).get_attribute("href")
...
def scrape_page(url, name="analytic/db"):
    ...
    with ChromeWebDriver() as driver:
        ...
        while True:
            ...
            vacancies = [
                parse(vacancy) for vacancy in tqdm(vacancies, desc=f"Scraping {name}")
            ]
